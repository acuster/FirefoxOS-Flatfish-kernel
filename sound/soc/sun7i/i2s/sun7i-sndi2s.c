...
static int sun7i_sndi2s_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	#ifdef ENFORCE_RATES
...
static void sun7i_sndi2s_shutdown(struct snd_pcm_substream *substream)
{
	mutex_lock(&clk_lock);
	clk_users -= 1;
	if (clk_users == 0) {
...
static struct snd_soc_ops sun7i_sndi2s_ops = {
	.startup 		= sun7i_sndi2s_startup,
	.shutdown 		= sun7i_sndi2s_shutdown,
