...
static int sun7i_sndspdif_startup(struct snd_pcm_substream *substream)
{
	int ret = 0;
	#ifdef ENFORCE_RATES
...
static void sun7i_sndspdif_shutdown(struct snd_pcm_substream *substream)
{
	mutex_lock(&clk_lock);
	clk_users -= 1;
	if (clk_users == 0) {
...
static int sun7i_sndspdif_hw_params(struct snd_pcm_substream *substream,
...
static struct snd_soc_ops sun7i_sndspdif_ops = {
	.startup 	= sun7i_sndspdif_startup,
	.shutdown 	= sun7i_sndspdif_shutdown,
	.hw_params 	= sun7i_sndspdif_hw_params,
};

static struct snd_soc_dai_link sun7i_sndspdif_dai_link = {
	.name 			= "SPDIF",
	.stream_name 	= "SUN7I-SPDIF",
	.cpu_dai_name 	= "sun7i-spdif.0",
	.codec_dai_name = "sndspdif",
	.platform_name 	= "sun7i-spdif-pcm-audio.0",
	.codec_name 	= "sun7i-spdif-codec.0",
	.ops 			= &sun7i_sndspdif_ops,
};

static struct snd_soc_card snd_soc_sun7i_sndspdif = {
	.name 		= "sndspdif",
	.dai_link 	= &sun7i_sndspdif_dai_link,
	.num_links 	= 1,
};

static struct platform_device *sun7i_sndspdif_device;
...
static int __init sun7i_sndspdif_init(void)
{
    ...
    if (spdif_used) {
		...
		if(!sun7i_sndspdif_device)
			return -ENOMEM;

		platform_set_drvdata(sun7i_sndspdif_device, &snd_soc_sun7i_sndspdif);

		ret = platform_device_add(sun7i_sndspdif_device);
		if (ret) {
			platform_device_put(sun7i_sndspdif_device);
		}
	} else {
		printk("[SPDIF]sun7i_sndspdif cannot find any using configuration for controllers, return directly!\n");
        return 0;
	}
...
static void __exit sun7i_sndspdif_exit(void)
{
	if (spdif_used) {
		spdif_used = 0;
		platform_device_unregister(sun7i_sndspdif_device);
	}

}

module_init(sun7i_sndspdif_init);
module_exit(sun7i_sndspdif_exit);

MODULE_AUTHOR("ALL WINNER");
MODULE_DESCRIPTION("SUN7I_SNDSPDIF ALSA SoC audio driver");
MODULE_LICENSE("GPL");
