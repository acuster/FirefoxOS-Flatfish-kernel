...
static void aw_set_clkevt_mode(enum clock_event_mode mode, struct clock_event_device *dev)
{
    TIMER_DBG("aw_set_clkevt_mode:%u\n", mode);
    switch (mode) {
...
{
	unsigned long flags;
    TIMER_DBG("aw_set_next_clkevt: %u\n", (unsigned int)delta);
...
static void __init aw_init_timer(void)
{
...
static int __init aw_clkevt_init(void)
{
    ...
    TIMER_DBG("set up all-winners clock event irq!\n");
    ...
    TIMER_DBG("register all-winners clock event device!\n");
