...
//赤“車?米?㏒?o赤GPIO?角1?米?那y?Y?芍11
typedef struct _normal_gpio_cfg
{
    __u8      port;                       //???迆o?
    __u8      port_num;                   //???迆?迆㊣角o?
    __s8      mul_sel;                    //1|?邦㊣角o?
    __s8      pull;                       //米?℅豕℅∩足?
    __s8      drv_level;                  //?y?‘?y?‘?邦芍|
    __s8      data;                       //那?3?米???
    __u8      reserved[2];                //㊣㏒芍???㏒?㊣㏒?∟????
}
normal_gpio_cfg;
...
typedef struct _boot0_private_head_t
{
        ...
        __s32                                           uart_port;              // UART?????¯㊣角o?
        normal_gpio_cfg             uart_ctrl[2];           // UART?????¯(米¯那?∩辰車??迆)那y?YD??⊿
        __s32                       enable_jtag;            // 1 : enable,  0 : disable
    normal_gpio_cfg                 jtag_gpio[5];           // ㊣㏒∩?JTAG米?豕?2?GPIOD??⊿
    normal_gpio_cfg             storage_gpio[32];       // ∩?∩⊿谷豕㊣? GPIOD??⊿
    char                        storage_data[512 - sizeof(normal_gpio_cfg) * 32];      // 車??∫㊣㏒芍?那y?YD??⊿
