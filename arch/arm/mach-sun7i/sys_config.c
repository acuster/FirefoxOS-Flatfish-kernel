...
#define PIO_BASE_PHYS		0x01c20800 	/* PA ~ PH */
#define RPIO_BASE_PHYS		0x01f02c00 	/* PL ~ PM */
#define PIO_BASE_VIRT		__va(PIO_BASE_PHYS)
#define RPIO_BASE_VIRT		__va(RPIO_BASE_PHYS)

#define GET_REGS_VBASE(n)	(((n) >= 1 && (n) <= 8) ? (PIO_BASE_VIRT) : (RPIO_BASE_VIRT))

extern char sys_cofig_data[];
extern char sys_cofig_data_end[];
#define __REG(x)                        (*(volatile unsigned int *)(x))

#define PIO_REG_CFG(n, i)               ((volatile unsigned int *)( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x00))
#define PIO_REG_DLEVEL(n, i)            ((volatile unsigned int *)( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x14))
#define PIO_REG_PULL(n, i)              ((volatile unsigned int *)( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x1C))
#define PIO_REG_DATA(n)                   ((volatile unsigned int *)( GET_REGS_VBASE(n) + ((n)-1)*0x24 + 0x10))

#define PIO_REG_CFG_VALUE(n, i)          __REG( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x00)
#define PIO_REG_DLEVEL_VALUE(n, i)       __REG( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x14)
#define PIO_REG_PULL_VALUE(n, i)         __REG( GET_REGS_VBASE(n) + ((n)-1)*0x24 + ((i)<<2) + 0x1C)
#define PIO_REG_DATA_VALUE(n)            __REG( GET_REGS_VBASE(n) + ((n)-1)*0x24 + 0x10)
...
u32 __gpio_request(user_gpio_set_t *gpio_list, __u32 group_count_max)
{
...
EXPORT_SYMBOL_GPL(__gpio_request);
/*
************************************************************************************************************
*
*                                             CSP_GPIO_Request_EX
...
u32 gpio_request_ex(char *main_name, const char *sub_name)  //设备申请GPIO函数扩展接口
{
    ...
    if(!sub_name){
        ...
        if(!script_parser_mainkey_get_gpio_cfg(main_name,gpio_list,gpio_count)){
            gpio_handle = __gpio_request(gpio_list, gpio_count);
            ...
            gpio_handle = __gpio_request(&one_gpio, 1);
        }
