 ...
 */
//static u32  gpio_g_pioMemBase = 0;
...
u32 sw_gpio_request(user_gpio_set_t *gpio_list, u32 group_count_max)
{
...
#if 1 // fix bug: may be memory overflow in "tmp_sys_gpio_data  = user_gpio_set + i" line below.
    ...
    *(int *)user_gpio_buf = real_gpio_count;                                           //保存有效的GPIO个数
	...
	/* config pio reg */
	if(0 != sw_gpio_setcfg(pio_index, tmp_user_gpio_data->mul_sel)) {
		usign = __LINE__;
		goto End;
	}
	if(0 != sw_gpio_setpull(pio_index, tmp_user_gpio_data->pull)) {
		usign = __LINE__;
		goto End;
	}
	if(0 != sw_gpio_setdrvlevel(pio_index, tmp_user_gpio_data->drv_level)) {
		usign = __LINE__;
		goto End;
	}
#else
...
u32 sw_gpio_request(user_gpio_set_t *gpio_list, u32 group_count_max)
{
    ...
    user_gpio_set_t  *tmp_user_gpio_data;
    u32                real_gpio_count = 0, first_port;                      //保存真正有效的GPIO的个数
    u32               tmp_group_func_data = 0;
    u32               tmp_group_pull_data = 0;
    u32               tmp_group_dlevel_data = 0;
    u32               tmp_group_data_data = 0;
    u32               func_change = 0, pull_change = 0;
    u32               dlevel_change = 0, data_change = 0;
    volatile u32  *tmp_group_func_addr = NULL, *tmp_group_pull_addr = NULL;
    volatile u32  *tmp_group_dlevel_addr = NULL, *tmp_group_data_addr = NULL;
    u32  port, port_num, port_num_func, port_num_pull;
    u32  pre_port = 0x7fffffff, pre_port_num_func = 0x7fffffff;
    u32  pre_port_num_pull = 0x7fffffff;
    s32  i, tmp_val;
...
s32 sw_gpio_release(u32 p_handler, s32 if_release_to_default_status)
{
	...
	/* set pull with hardware_gpio_status.pull */
	if(0 != sw_gpio_setpull(pio_index, tmp_sys_gpio_data->hardware_gpio_status.pull)) {
		usign = __LINE__;
		goto End;
	}

	/* set drv_level with hardware_gpio_status.drv_level */
	if(0 != sw_gpio_setdrvlevel(pio_index, tmp_sys_gpio_data->hardware_gpio_status.drv_level)) {
		usign = __LINE__;
		goto End;
	}
#else
...
s32 sw_gpio_release(u32 p_handler, s32 if_release_to_default_status)
{
    ...
    u32               tmp_group_dlevel_data = 0;
    volatile u32     *tmp_group_func_addr = NULL,   *tmp_group_pull_addr = NULL;
    volatile u32     *tmp_group_dlevel_addr = NULL;
    u32               port, port_num, port_num_pull, port_num_func;
    u32               pre_port = 0x7fffffff, pre_port_num_func = 0x7fffffff, pre_port_num_pull = 0x7fffffff;
    u32               i, tmp_val;
...
s32  sw_gpio_get_all_pin_status(u32 p_handler, user_gpio_set_t *gpio_status, u32 gpio_count_max, u32 if_get_from_hardware)
{
	...
	/* get pull val */
	usign = sw_gpio_getpull(pio_index);
	if(GPIO_PULL_INVALID == usign) {
		usign = __LINE__;
		goto End;
	}
	script_gpio->pull = usign;

	/* get drv level val */
	usign = sw_gpio_getdrvlevel(pio_index);
	if(GPIO_DRVLVL_INVALID == usign) {
		usign = __LINE__;
		goto End;
	}
	script_gpio->drv_level = usign;

	/* get data val if cfg is input/output */
	usign = sw_gpio_getcfg(pio_index);
	if(GPIO_CFG_INVALID == usign) {
		usign = __LINE__;
		goto End;
	}
	script_gpio->mul_sel = usign;
	if(usign <= 1)
...
s32  sw_gpio_get_all_pin_status(u32 p_handler, user_gpio_set_t *gpio_status, u32 gpio_count_max, u32 if_get_from_hardware)
{
    ...
    system_gpio_set_t  *user_gpio_set, *tmp_sys_gpio_data;
    user_gpio_set_t  *script_gpio;
    u32               port_num_func, port_num_pull;
    volatile u32     *tmp_group_func_addr = NULL, *tmp_group_pull_addr;
    volatile u32     *tmp_group_data_addr, *tmp_group_dlevel_addr;
    u32               port, port_num;
    u32               pre_port = 0x7fffffff, pre_port_num_func = 0x7fffffff, pre_port_num_pull = 0x7fffffff;
    u32               i;
...
s32  sw_gpio_set_one_pin_status(u32 p_handler, user_gpio_set_t *gpio_status, const char *gpio_name, u32 if_set_to_current_input_status)
{
    ...
    system_gpio_set_t  *user_gpio_set, *tmp_sys_gpio_data;
    user_gpio_set_t     script_gpio;
    volatile u32     *tmp_addr;
    u32               port_num_func, port_num_pull;
    u32               port, port_num;
    u32               i, reg_val, tmp_val;
...
s32  sw_gpio_set_one_pin_io_status(u32 p_handler, u32 if_set_to_output_status, const char *gpio_name)
{
    ...
    u32               group_count_max;                                //最大GPIO个数
    system_gpio_set_t  *user_gpio_set = NULL, *tmp_sys_gpio_data;
    volatile u32      *tmp_group_func_addr = NULL;
    u32               port, port_num, port_num_func;
    u32                i, reg_val;
...
s32  sw_gpio_set_one_pin_pull(u32 p_handler, u32 set_pull_status, const char *gpio_name)
{
    ...
    u32               port, port_num, port_num_pull;
    u32                i, reg_val;
    //检查传进的句柄的有效性
    if(!p_handler)
    {
...
s32  sw_gpio_set_one_pin_driver_level(u32 p_handler, u32 set_driver_level, const char *gpio_name)
{
    ...
    system_gpio_set_t  *user_gpio_set = NULL, *tmp_sys_gpio_data;
    volatile u32      *tmp_group_dlevel_addr = NULL;
    u32               port, port_num, port_num_dlevel;
    u32                i, reg_val;
    //检查传进的句柄的有效性
    if(!p_handler)
    {
...
s32  sw_gpio_read_one_pin_value(u32 p_handler, const char *gpio_name)
{
    ...
    u32               group_count_max;                                //最大GPIO个数
    system_gpio_set_t  *user_gpio_set = NULL, *tmp_sys_gpio_data;
    u32               port, port_num, port_num_func, func_val;
    u32                i, reg_val;
    //检查传进的句柄的有效性
    if(!p_handler)
    {
...
s32  sw_gpio_write_one_pin_value(u32 p_handler, u32 value_to_gpio, const char *gpio_name)
{
    ...
    u32               group_count_max;                                //最大GPIO个数
    system_gpio_set_t  *user_gpio_set = NULL, *tmp_sys_gpio_data;
    volatile u32     *tmp_group_data_addr = NULL;
    u32               port, port_num, port_num_func, func_val;
    u32                i, reg_val;
    //检查传进的句柄的有效性
    if(!p_handler)
