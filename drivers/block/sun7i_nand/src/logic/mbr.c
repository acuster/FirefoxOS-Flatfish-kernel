...
int part_secur[NAND_MAX_PART_CNT];
...
int mbr2disks(struct nand_disk* disk_array)
{
	...
	for(part_cnt = 0; part_cnt < mbr->PartCount && part_cnt < NAND_MAX_PART_CNT; part_cnt++)
	{
	    ...
	    {
			PRINT("The %d disk name = %s, class name = %s, disk size = %x\n", part_index, mbr->array[part_cnt].name,
			...
			part_index ++;
			PRINT("The %d disk offset = %lu\n", part_index - 1, disk_array[part_index - 1].offset);
	...
	{
		NandPartTable.start_sec[part_cnt] = disk_array[part_cnt].offset;
		NandPartTable.sec_cnt[part_cnt] = disk_array[part_cnt].size;
		NandPartTable.part_type[part_cnt] = part_secur[part_cnt];
	}

	//for debug info
	#if 0
	DBUG_MSG("NandPartTable: \n");
	DBUG_MSG("part cnt: %d\n",NandPartTable.part_cnt);
	for(part_cnt = 0; part_cnt<NandPartTable.part_cnt; part_cnt++)
	{
		//for debug
		if(part_cnt == NandPartTable.part_cnt -1)
			part_secur[part_cnt] = LSB_TYPE;

		NandPartTable.part_type[part_cnt] = part_secur[part_cnt];
		DBUG_MSG("part_num: %d, start_sec: %x, sec_cnt: %x, type: %x\n", part_cnt, NandPartTable.start_sec[part_cnt], NandPartTable.sec_cnt[part_cnt], NandPartTable.part_type[part_cnt]);
	}
    #endif
