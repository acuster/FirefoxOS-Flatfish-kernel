...
__s32 BSP_disp_layer_release(__u32 sel, __u32 hid)
{
 ...
 /*
                    if(BSP_disp_deu_get_enable(sel, hid))
                    {
                        BSP_disp_deu_enable(sel, hid, FALSE);
                    }
*/
...
__s32 BSP_disp_layer_set_para(__u32 sel, __u32 hid,__disp_layer_info_t *player)
{
	...
	BSP_disp_cfg_start(sel);
		...
		else
		{
            ...
            __inf("layer_set_para\n");
            if(BSP_disp_cmu_layer_get_enable(sel, hid))
            {
                __inf("layer_set_para, sel=%d, hid=%d,en=%d\n", sel, hid,BSP_disp_cmu_layer_get_enable(sel, hid));
