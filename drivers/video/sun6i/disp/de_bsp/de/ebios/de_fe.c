...
//
//  Description :  display engine scaler base functions implement
//
//  History       :
                               ...
                               __scal_out_size_t *out_size, __scal_out_type_t *out_type, __u8 smth_mode)
{
    ...
    if((count&0x01) == 0)
    {
        *(volatile __u32*)0xf1e00004 = (*(volatile __u32*)0xf1e00004) | 0x3;
    }
    count++;
#endif
    in_w0 = in_size->scal_width;
    ...
    for(i=0; i<32; i++)
    {
        ...
        scal_dev[sel]->ch0_horzcoef0[i].dwval = fir_tab[(ch0h_fir_coef_addr>>2) + 2*i];
        scal_dev[sel]->ch0_horzcoef0[i].dwval = fir_tab[(ch0h_fir_coef_addr>>2) + 2*i];
        __debug("reg=0x%08x,fir_tab=0x%08x;\t",scal_dev[sel]->ch0_horzcoef0[i].dwval, fir_tab[(ch0h_fir_coef_addr>>2) + 2*i]);
		scal_dev[sel]->ch0_horzcoef1[i].dwval = fir_tab[(ch0h_fir_coef_addr>>2) + 2*i + 1];
        __debug("reg=0x%08x,fir_tab=0x%08x;\n",scal_dev[sel]->ch0_horzcoef1[i].dwval, fir_tab[(ch0h_fir_coef_addr>>2) + 2*i + 1]);
        scal_dev[sel]->ch0_vertcoef[i].dwval  = fir_tab[(ch0v_fir_coef_addr>>2) + i];
		__debug("reg=0x%08x,fir_tab=0x%08x;\t",scal_dev[sel]->ch0_vertcoef[i].dwval, fir_tab[(ch0v_fir_coef_addr>>2) + i]);
        scal_dev[sel]->ch1_horzcoef0[i].dwval = fir_tab[(ch1h_fir_coef_addr>>2) + 2*i];
		__debug("reg=0x%08x,fir_tab=0x%08x;\n",scal_dev[sel]->ch1_horzcoef0[i].dwval, fir_tab[(ch1h_fir_coef_addr>>2) + 2*i]);
        scal_dev[sel]->ch1_horzcoef1[i].dwval = fir_tab[(ch1h_fir_coef_addr>>2) + 2*i + 1];
        __debug("reg=0x%08x,fir_tab=0x%08x;\t",scal_dev[sel]->ch1_horzcoef1[i].dwval, fir_tab[(ch1h_fir_coef_addr>>2) + 2*i + 1]);
        scal_dev[sel]->ch1_vertcoef[i].dwval  = fir_tab[(ch1v_fir_coef_addr>>2) + i];
        __debug("reg=0x%08x,fir_tab=0x%08x;\n",scal_dev[sel]->ch1_vertcoef[i].dwval, fir_tab[(ch1v_fir_coef_addr>>2) + i]);
...
{
    __inf("DE_SCAL_Set_Out_Format: seq=0x%08x, fmt=0x%08x\n", out_type->byte_seq, out_type->fmt);
	scal_dev[sel]->output_fmt.bits.byte_seq = out_type->byte_seq;
    scal_dev[sel]->output_fmt.bits.data_fmt = out_type->fmt;
    __inf("DE_SCAL_Set_Out_Format: seq=0x%08x, fmt=0x%08x\n", scal_dev[sel]->output_fmt.bits.byte_seq, scal_dev[sel]->output_fmt.bits.data_fmt);
    __inf("0xf1e0005c:0x%08x\n", *(volatile __u32*)0xf1e0005c);
    return 0;
}
