...
__u32 tcon0_cfg(__u32 sel, __panel_para_t * info)
{
	__u32 vblank_len;
	__u32 lcd_if_reg = 0;
	...
	LCDC_INIT_BIT(sel, LCDC_CTL_OFF,0x0ffffff0,(lcd_if_reg <<24) | (info->lcd_rb_swap<< 23) | (0<<20) | (info->start_delay<<4));
...
{
        __tcon1_cfg_t tcon_cfg;

        tcon_cfg.b_interlace = 0;
        tcon_cfg.b_rgb_internal_hd = 0;
        tcon_cfg.b_rgb_remap_io = 1;//rgb
        tcon_cfg.b_remap_if = 1;	//remap tcon1 to io
        tcon_cfg.src_x = info->lcd_x;
        tcon_cfg.src_y = info->lcd_y;
        tcon_cfg.scl_x = info->lcd_x;
        tcon_cfg.scl_y = info->lcd_y;
        tcon_cfg.out_x = info->lcd_x;
        tcon_cfg.out_y = info->lcd_y;
        tcon_cfg.ht = info->lcd_ht;
        tcon_cfg.hbp = info->lcd_hbp;
        tcon_cfg.vt = info->lcd_vt;
        tcon_cfg.vbp = info->lcd_vbp;
        tcon_cfg.vspw = info->lcd_vspw;
        tcon_cfg.hspw = info->lcd_hspw;
        tcon_cfg.io_pol = info->lcd_io_cfg0;
        tcon_cfg.io_out = info->lcd_io_cfg1;

        tcon1_cfg(sel, &tcon_cfg);
