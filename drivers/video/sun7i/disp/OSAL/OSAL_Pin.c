...
__hdle OSAL_GPIO_Request(user_gpio_set_t *gpio_list, __u32 group_count_max)
{
    ...
    return sw_gpio_request(gpio_list, group_count_max);
}

__hdle OSAL_GPIO_Request_Ex(char *main_name, const char *sub_name)
{
    return sw_gpio_request_ex(main_name, sub_name);
}
...
__s32 OSAL_GPIO_Release(__hdle p_handler, __s32 if_release_to_default_status)
{
    ...
    sw_gpio_release(p_handler, if_release_to_default_status);
...
{
    return sw_gpio_get_all_pin_status(p_handler, gpio_status, gpio_count_max, if_get_from_hardware);
}
...
{
    return sw_gpio_get_one_pin_status(p_handler, gpio_status,gpio_name,if_get_from_hardware);
}
...
{
    return sw_gpio_set_one_pin_status(p_handler, gpio_status, gpio_name, if_set_to_current_input_status);
}

__s32 OSAL_GPIO_DevSetONEPIN_IO_STATUS(u32 p_handler, __u32 if_set_to_output_status, const char *gpio_name)
{
    return sw_gpio_set_one_pin_io_status(p_handler, if_set_to_output_status, gpio_name);
}

__s32 OSAL_GPIO_DevSetONEPIN_PULL_STATUS(u32 p_handler, __u32 set_pull_status, const char *gpio_name)
{
    return sw_gpio_set_one_pin_pull(p_handler, set_pull_status, gpio_name);
}

__s32 OSAL_GPIO_DevREAD_ONEPIN_DATA(u32 p_handler, const char *gpio_name)
{
    return sw_gpio_read_one_pin_value(p_handler, gpio_name);
}

__s32 OSAL_GPIO_DevWRITE_ONEPIN_DATA(u32 p_handler, __u32 value_to_gpio, const char *gpio_name)
{
    return sw_gpio_write_one_pin_value(p_handler, value_to_gpio, gpio_name);
}
