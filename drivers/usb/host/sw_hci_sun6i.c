...
static u32 usbc_base[4] 			= {SW_VA_USB0_IO_BASE, SW_VA_USB1_IO_BASE, SW_VA_USB2_IO_BASE, SW_VA_USB3_IO_BASE};
static u32 ehci_irq_no[4] 			= {0, SW_INT_SRC_EHCI0, SW_INT_SRC_EHCI1};
static u32 ohci_irq_no[4] 			= {0, SW_INT_SRC_OHCI0, SW_INT_SRC_OHCI1, SW_INT_SRC_OHCI2};

u32 usb1_drv_vbus_Handle = 0;
u32 usb2_drv_vbus_Handle = 0;
u32 usb3_drv_vbus_Handle = 0;
...
static u32 usb3_enable_configure_cnt = 0;
#endif
/*
*******************************************************************************
*                     get_usb_cfg
...
static void hci_port_configure(struct sw_hci_hcd *sw_hci, u32 enable)
{
	...
	if(sw_hci->usbc_no == 1){
		...
		usbc_sdram_hpcr = SW_SDRAM_REG_HPCR_USB3;
		reg_value = USBC_Readl(sw_hci->sdram_vbase + usbc_sdram_hpcr);
		if(enable && usb3_enable_configure_cnt == 0){
			reg_value |= (1 << SW_SDRAM_BP_HPCR_ACCESS_EN);
		}else if(!enable && usb3_enable_configure_cnt == 1){
			reg_value &= ~(1 << SW_SDRAM_BP_HPCR_ACCESS_EN);
		}
		USBC_Writel(reg_value, (sw_hci->sdram_vbase + usbc_sdram_hpcr));

        if(enable){
            usb3_enable_configure_cnt++;
        }else{
            usb3_enable_configure_cnt--;
        }
	}else{
...
static int __init sw_hci_sun6i_init(void)
{
    ...
    usb3_drv_vbus_Handle = alloc_pin(&sw_ohci2.drv_vbus_gpio_set);
    if(usb3_drv_vbus_Handle == 0){
        DMSG_PANIC("ERR: usb2 alloc_pin failed\n");
        goto failed0;
    }
...
static void __exit sw_hci_sun6i_exit(void)
{
    ...
    free_pin(usb3_drv_vbus_Handle);
    usb3_drv_vbus_Handle = 0;
