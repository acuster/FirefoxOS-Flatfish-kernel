/*
   ...
   2012-Jun-13	Magic            Modified for EVB
*/
...
{
	if (on) {
		BT_SLEEP_DBG("hsuart_power HS UART ON");
		bsi->uport->ops->set_mctrl(bsi->uport, TIOCM_RTS);
	} else {
		BT_SLEEP_DBG("hsuart_power HS UART OFF");
...
static int bluesleep_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	BT_SLEEP_DBG("bluesleep suspending");
	return 0;
}

static int bluesleep_resume(struct platform_device *pdev, pm_message_t state)
{
	BT_DBG("bluesleep resuming");
	BT_SLEEP_DBG("bluesleep resumeing");
	if(bsi->uport != NULL &&
		__gpio_get_value(bsi->host_wake) )
	{
		BT_DBG("bluesleep resume from hostwake...");
		BT_SLEEP_DBG("bluesleep resume from hostwake...");
		hsuart_power(1);
	}
	return 0;
}
