		...
		struct device_attribute *attr, char *buf)
{
	...
	dprintk(DEBUG_CONTROL_INFO, "%d, %s\n", atomic_read(&afa750_data.suspended), __FUNCTION__);
	return sprintf(buf, "%d\n", atomic_read(&afa750_data.suspended));
...
static ssize_t afa50_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	dprintk(DEBUG_CONTROL_INFO, "%d, %s\n", afa750_idev->poll_interval, __FUNCTION__);
	return sprintf(buf, "%d\n", afa750_idev->poll_interval);
...
static DEVICE_ATTR(delay, 0666,afa50_delay_show, afa750_delay_store);
...
{
	if(atomic_read(&afa750_data.suspended) == 1){
