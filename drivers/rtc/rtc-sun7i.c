...
retry_get_time:
	...
	RTC_DBG("%s,line:%d, rtc_tm->tm_year:%d\n", __func__, __LINE__, rtc_tm->tm_year);
...
static int sunxi_rtc_settime(struct device *dev, struct rtc_time *tm)
{
    ...
    */
	...
	if(leap_year > 2100 || leap_year < 1970) {
		dev_err(dev, "rtc only supports 130（1970～2100） years\n");
		return -EINVAL;
	}
