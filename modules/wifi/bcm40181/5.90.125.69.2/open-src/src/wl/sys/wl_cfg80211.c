...
#define WL_TRACE(a) printf("%s ", __FUNCTION__); printk a
...
#ifdef DHD_DEBUG
	if ( module_remove ){
		printf("%s: module_remove is 1. skip.\n", __FUNCTION__);
		return NULL;
	}
#endif
...
/*
                        ...
                        case WLAN_AKM_SUITE_WAPI_PSK:
                                WL_ERR((" * wl_set_key_mgmt, WLAN_AKM_SUITE_WAPI_PSK"));
                                val = WAPI_AUTH_PSK;
                                break;
                        case WLAN_AKM_SUITE_WAPI_CERT:
                                WL_ERR((" * wl_set_key_mgmt, WLAN_AKM_SUITE_WAPI_CERT"));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (get_mode_by_netdev(wl, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(wl, e, ndev)) {
			...
			printf("connect failed e->status 0x%x", (int)ntoh32(e->status));
