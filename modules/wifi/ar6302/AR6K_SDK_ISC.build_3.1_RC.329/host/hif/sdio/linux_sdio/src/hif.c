...
extern int sw_mci_check_r1_ready(struct mmc_host *mmc);
static int __HIFCheckR1Ready(HIF_DEVICE *device, unsigned ms)
{
	u32 timeout = jiffies + msecs_to_jiffies(ms);
	AR_DEBUG_ASSERT(device != NULL);
	AR_DEBUG_ASSERT(device->func != NULL);

	while (!sw_mci_check_r1_ready(device->func->card->host)
		&& jiffies < timeout) {
		AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("HIF wait r1 ready ...\n"));
	}

	return jiffies <  timeout ? 1 : 0;
}
#endif
...
#if defined(CONFIG_MMC_SUN6I) || defined(CONFIG_MMC_SUN7I)
		/* winner's support, to check r1 response ready status */
            if (!__HIFCheckR1Ready(device, 300))
                ret = -1;
#endif
